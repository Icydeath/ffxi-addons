...
function cast.song(str,ta,buffs,recasts,JA_WS_lock)
    ...
    elseif ta ~= '<me>' and not buffs.pianissimo then 
		...
		if settings.gearswap_dummy_cmd and has_value(settings.dummy, str) then -- (Icy) added this so it will toggle my dummy set in Gearswap.
			windower.send_command('gs c '..settings.gearswap_dummy_cmd)
		end
        cast.MA(str,ta)
    end
end
