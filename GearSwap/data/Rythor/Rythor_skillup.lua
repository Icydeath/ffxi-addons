-- Based off of [version 2.0.0.0 final]
-- current version: 2.0.0.2

--[[ Icydeath's additions:
	
	2.0.0.2
		> Changed if using healing magic and the convert ja is used it won't use large cure spells to recover HP 
			but will relay on the healing magic spells set by the user to recover the missing HP
		> Fixed buff id for spell refresh.
		
	2.0.0.1
		> Added the ability to cast the spell Refresh when the buff is not active. Button added to the menu 'Use Spell Refresh'
		> Added the ability to convert when unable to cast a spell due to low MP. Button added to the menu 'Use Convert'
			> Convert addon: Added the ability to self cure when HP below 50% if 'Use Convert' is enabled.
]]


--[[ NOTE: To remove annoying click error when click off ffxi and clicking back to ffxi.
To temp fix this I added the below to the function 'texts.hover' in the file 'addons\libs\texts.lua'  
...
***TO THIS***
	function texts.hover(t, x, y)
		-- This check was added by Icydeath
		if not t then
			return false
		end
...
You will have to do a //reload luacore and //gs l skillup.lua after making the changes to fix the annoying error.
]]
...
sets.brd = {
    wind_inst = {range="Cornette"},--put your wind instrument here
    string_inst = {range="Lamia Harp"},--put your string instrument here
}
sets.Idle = {}
sets.Resting = {}
...
function get_sets()
    ...
    -- added SREF & use_sref (for spell refresh) and CONV & use_conv (for convert) to gs_skillup
	gs_skillup = {color={GEO=true,HEL=true,ENH=true,NIN=true,SIN=true,BLU=true,SMN=true,STOP=true,DOWN=true,LOG=true,TRUST=true,TEST=true,REF=true,ITEM=true,SREF=true,CONV=true},
                skill_ups={},total_skill_ups=0,skill={},use_trust=false,use_item=false,use_geo=false,test_mode=false,test_brd="Wind",use_sref=false,use_conv=false,skipped_spells=T{}}
...
function precast(spell)
    ...
    if spell and spell.mp_cost > player.mp then
        if gs_skillup.skipped_spells:contains(spell.name) then
			...
			-- Convert: added by icy
			if gs_skillup.use_conv and player.hpp > 50 then 
				if (windower.ffxi.get_ability_recasts()[49] == 0) then
					cancel_spell()
					send_command('input /ja "'..res.job_abilities[83][gearswap.language]..'" <me>')
					return
				end
			end
	...
	-- Self Curing: Added by icy
	if gs_skillup.use_conv 
			and player.hpp < 50 
			and not gs_skill.skillup_spells:contains('Cure')
			and not gs_skill.skillup_spells:contains('Cure II')
			and not gs_skill.skillup_spells:contains('Cure III')
			and not gs_skill.skillup_spells:contains('Cure IV')
			and not gs_skill.skillup_spells:contains('Cure V')
			and not gs_skill.skillup_spells:contains('Cure VI') then 
        if spell_usable(res.spells[4]) then
			if spell.en ~= "Cure IV" then
				cancel_spell()
				send_command('input /ma "'..res.spells[4][gearswap.language]..'" <me>')
			end
		elseif spell_usable(res.spells[3]) then
			if spell.en ~= "Cure III" then
				cancel_spell()
				send_command('input /ma "'..res.spells[3][gearswap.language]..'" <me>')
			end
		elseif spell_usable(res.spells[2]) then
			if spell.en ~= "Cure II" then
				cancel_spell()
				send_command('input /ma "'..res.spells[2][gearswap.language]..'" <me>')
			end
		elseif spell_usable(res.spells[1]) then
			if spell.en ~= "Cure" then
				cancel_spell()
				send_command('input /ma "'..res.spells[1][gearswap.language]..'" <me>')
			end
		end
        return
	end
	-- Self Refresh: Added by icy
	if gs_skillup.use_sref and not buffactive[43] and spell_usable(res.spells[109]) then 
    ...
    if gs_skillup.use_geo and player.main_job == "GEO" and spell_usable(res.spells[800]) and not pet.isvalid then
    ...
    if gs_skillup.use_geo and player.sub_job == "GEO" and spell_usable(res.spells[770]) and buffactive[541] ~= (gs_skillup.use_trust and 2 or 1) then
...
function self_command(command)
	...
	elseif command == 'setsref' then 					-- added by icy
        gs_skillup.use_sref = not gs_skillup.use_sref
	elseif command == 'setconv' then 					-- added by icy
        gs_skillup.use_conv = not gs_skillup.use_conv
...
function initialize(text, settings, a)
    if a == 'window' then
		...
		if gs_skillup.use_sref then -- added by icy
            properties:append('\\crUsing Spell Refresh')
        end
		if gs_skillup.use_conv then -- added by icy
            properties:append('\\crUsing Convert')
        end
    ...
    if a == 'button' then
		...
		properties:append('${SREFc}') --added by icy
		properties:append('${CONVc}') --added by icy
        text:clear()
        text:append(properties:concat('\n'))
    end
end
function updatedisplay()
    local info = {}
		...
		info.SREFc = (gs_skillup.color.SREF and  'Use Spell Refresh' or '\\cs(255,0,0)Use Spell Refresh\\cr') -- added by icy
		info.CONVc = (gs_skillup.color.CONV and  'Use Convert' or '\\cs(255,0,0)Use Convert\\cr') -- added by icy
        ...
        elseif button:hover(x, y) and button:visible() then
            ...
            for i, v in ipairs(location) do
                local switch = {[1]="TRUST",[2]='REF',[3]='ITEM',[4]="GEO",[5]="HEL",[6]="ENH",[7]="NIN",[8]="SIN",[9]="BLU",[10]="SMN",[11]="STOP",[12]="DOWN",
                                [13]="LOG",[14]="SREF",[15]="CONV",[16]="TEST"} -- added SREF & CONV by icy
        ...
        if button:hover(x, y) and button:visible() then
            for i, v in ipairs(location) do
                local switchb = {[1]="settrust",[2]="setgeo",[3]="setitem",[4]="start Geomancy",[5]="start Healing",[6]="start Enhancing",[7]="start Ninjutsu",
                                ...
                                [14]="setsref",[15]="setconv",[16]="changeinstrament"} -- added setsref & setconv by icy
                if hy > location[i].ya and hy < location[i].yb then
