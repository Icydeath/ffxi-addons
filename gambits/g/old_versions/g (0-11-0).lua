...
function zone_refresh()
  ...
  my_id = player.id
  my_name = player.name
  party, party_members, party_locations, party_distances, clusters = {}, {}, {}, {}, {}
  add_party_entry(my_id, my_name)
  ...
  for key,value in pairs(party_info) do
    ...
    elseif (string.find(key, "leader")) then
      ...
      if (value.mob) then
        ...
        elseif (party_members[mob_id] == nil) then
          -- ID slot exists, but was created by party_buff packet.
          local old_id = party_members[mob_name]
          party_members[old_id] = nil
          party[old_id] = nil
          party_members[mob_id] = mob_name
        end
...
function parse_party_buffs(data)
  ...
  while ((player_id ~= 0) and (player_i <= 5)) do
    if (party[player_id] == nil) then
      add_party_entry(player_id, '_')
    end
...
function process_selector(selector, trigger, trigger_arg, gambit_num)
  ...
  if ((selector == "SELF") or (selector == "PARTY") or (selector == "CLUSTER") or (selector == "ENEMY")) then
    if (((selector == "SELF") and valid_self_triggers[trigger]) or
      (((selector == "PARTY") or (selector == "CLUSTER")) and valid_party_triggers[trigger]) or
