...
function get_targetted_enemy()
  ...
  return {['casting']=false,['readying']=false,['id']=0}
end
...
function process_updated_party_list()
  ...
  if (last_party_list_processed_time ~= time) then
    party_locations, party_distances, clusters = get_party_distances()
...
function get_party_distances()
  local distances, party_locations, clusters = {}, {}, {}
  local mob = false
  local distance = -9999
  ...
  return party_locations, distances, clusters
end
...
-- Checks to see if we've somehow locked cta to be false.
-- Note: I really shouldn't need to do it this way. I
-- need to figure out and fix the root problem later.
---------------------------------------------------------------------
function check_if_frozen(time)
  if (time == last_reaction_time) then
    print('Was frozen')
    cta(true)
  end
end
...
function ability_ready(ability_type, ability_id)
  ...
  elseif (ability_type == 'ITEM') then
    ...
    elseif (ability_type == 'MA') then
      ...
      recast_id = ability.recast_id
      if ((party[my_id].mp < ability.mp_cost) or has_status(my_id,6)) then
        return false
      end
    end
...
function check_gambit_list(gambits, num_gambits)
  ...
  if (can_take_action and gambits_enabled and (has_status(my_id, 10)~= true)) then
...
function satisfies_condition(target, condition, c_arg, multiple, cluster)
  ...
  if (target == 'AND') then
    ...
    local switch = {
      ...
      ["CAN_MB"] = function(target, c_arg) return (check_MB_elements(get_targetted_enemy()[1].id,c_arg)) end,
...
function react(ability, target)
  ...
  cta(false)
  --last_reaction_time = windower.ffxi.get_info().time
  --coroutine.close(cta_check_routine)
  --cta_check_routine = coroutine.schedule(function() check_if_frozen(last_reaction_time) end,30)
...
function process_selector(selector, trigger, trigger_arg, gambit_num)
  ...
  local triggers_with_raw_args = S{"HPP <=","HPP >","MPP <=","TP >=","NOT_ENGAGED","NOT_ASSISTING","SHADOWS <","MOVES <","MOVES >=","NO_PET","CAN_SC","CAN_MB"}
...
function process_reaction(reaction, reaction_arg, gambit_num)
  ...
  if ((reaction == "ATTACK") or (reaction == "ASSIST") or (reaction == "SC")) then
    ...
    if (reaction == "JA") then
      res_table = job_abilities_by_en
    elseif ((reaction == "MA") or (reaction == "GEO")) then
      res_table = spells_by_en
...
unable_to_perform_messages = S{78, 198, 328, 41, 154, 313, 47, 48, 71, 72, 92, 128, 155, 190, 193, 217, 219, 316, 338, 348, 349, 445, 446}
...
my_id = player.id
my_name = player.name
party, party_members, party_jobs, party_locations, party_distances, clusters = {}, {}, {}, {}, {}, {}
party_jobs['tank'], party_jobs['melee'], party_jobs['ranged'], party_jobs['mage'] = {}, {}, {}, {}
...
cta_check_routine, last_reaction_time = false, false -- I really shouldn't need this. Need to fix actual root cause.
