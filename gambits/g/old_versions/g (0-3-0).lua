...
function add_status(target_id, status)
  if (in_party(target_id)) then
    party[target_id].statuses[status] = true
  else
...
function check_gambit_list()
  ...
  if (reaction == false) then
    reaction = check_gambit("ENEMY","PERFORMING",2696,"JA",207)
    --mob.performing = false
  end
  if (reaction == false) then
    reaction = check_gambit("SELF","TP >=",1000,"WS",31)
  end
  if (reaction == false) then
