...
function zone_refresh()
  ...
  movement_update_loop = coroutine.schedule(update_moving, 2)
  ...
  for key,value in pairs(party_info) do
    if (get_pt_party_keys[key]) then
      if (value.mob) then
        ...
        party[mob_id].is_trust = value.mob.is_npc
...
function update_party_list(data)
  ...
  if (party[id]) then
    -- Delayed because mob may not have appeared yet at party packet arrival
...
function refresh_party_status()
  ...
  for key,value in pairs(party_info) do
    if (get_pt_party_keys[key]) then
      if (value.mob) then
        ...
        if (sphere_trusts[mob_name]) then
          party[mob_id].hp = 0
          party[mob_id].hpp = 100
        end
      end
    end
  end
  ...
  party[my_id].num_maneuvers = 0
  party[my_id].maneuvers = {0, 0, 0, 0, 0, 0, 0, 0}
  status_locked = false
  for _,value in pairs(get_player_buffs) do
    ...
    elseif ((value >= 300) and (value <= 307)) then
      party[my_id].num_maneuvers = party[my_id].num_maneuvers + 1
      party[my_id].maneuvers[value - 299] = party[my_id].maneuvers[value - 299] + 1
...
function parse_party_buffs(data)
  ...
  local bitmask_add_value_byte, bitmask_add_value = 0, 0
  local orig_buff_value, buff_value = 0
  ...
  for player_i = 1,5 do
    ...
    player_buff_bitmask = buff_packet["Bit Mask ".. player_i]
    if (player_id ~= 0) then
      party_statuses[player_id] = {}
      for buff_i = 0,31 do
        buff_value = string.byte(player_buff_data, buff_i + 1)
        if (buff_value ~= 255) then
          bitmask_add_value_byte = player_buff_bitmask:byte(1+ math.floor(buff_i/4))
          bitmask_add_value = bit.band(3, bit.rshift(bitmask_add_value_byte, 2 * (buff_i % 4)))
          buff_value = buff_value + (bitmask_add_value * 256)
...
-- Determines if the character is currently moving (to block gambit checks)
---------------------------------------------------------------------
function check_movement()
  local me = windower.ffxi.get_mob_by_id(my_id)
  if me then
    if not ((me.x == my_last_x) and (me.y == my_last_y)) then
      my_last_x, my_last_y = me.x, me.y
      return true
    else
      return false
    end
...
function check_gambit_list()
  ...
  if (can_act() and not moving) then
    local gambits, num_gambits = determine_gambits()
    local me = windower.ffxi.get_player()
    if ((engaged_only == false) or (engaged_only and ((me.status == 1) or (callback)))) then
      if (chaining) then
        ...
        elseif (callback) then
          windower.send_command("ava taskdone")
          callback = false;
        end
      end
    end
...
function satisfies_condition(target, condition, c_arg, multiple, cluster)
  ...
  if (target == 'AND') then
    ...
    local switch = {
      ...
      ["NOT_STATUS"] = function(target, c_arg) return ((target.is_trust == false) and (has_status(target.id, c_arg) == false)) end,
      ...
      ["FIRE_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[1] < c_arg) end,
      ["ICE_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[2] < c_arg) end,
      ["WIND_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[3] < c_arg) end,
      ["EARTH_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[4] < c_arg) end,
      ["THUNDER_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[5] < c_arg) end,
      ["WATER_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[6] < c_arg) end,
      ["LIGHT_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[7] < c_arg) end,
      ["DARK_MANEUVERS <"] = function(target, c_arg) return (target.maneuvers[8] < c_arg) end,
      ["MANEUVERS <"] = function(target, c_arg) return (target.num_maneuvers < c_arg) end,
      ["MOVES >="] = function(target, c_arg) return (target.moves >= c_arg) end,
      ["NO_PET"] = function(target, c_arg) return (target.pet == false) end,
      ["PET_IDLE"] = function(target, c_arg) return (target.pet and (target.pet.status == 0)) end,
      ["PET_HPP <="] = function(target, c_arg) return (target.pet and (target.pet.hpp <= c_arg)) end,
        ...
        if (cluster_size >= 3) then
          ...
          while (cluster_member_i <= cluster_size + 2) do
            if(switch[condition](party[cluster_array[cluster_member_i]], c_arg)) then
              if (party[cluster_array[cluster_member_i]].hp > 0) then
...
function react(ability, target)
  ...
  local command, cast_time_length = '', 5
  ...
  elseif (target == "ITEM") then
    ...
    if (ability.cast_time) then
      if (has_status(my_id, 286)) then -- Addle
        cast_time_length = (ability.cast_time * 1.3) + 5
      else
        cast_time_length = ability.cast_time + 5
      end
    end
  ...
  cta(false)
  cta_ticket = cta_ticket + 1
  local to_check_cta_ticket = cta_ticket
  coroutine.schedule(function()  check_cta_ticket(to_check_cta_ticket) end, cast_time_length)
...
function determine_gambits()
  ...
  elseif num_my_gambits > 0 then
    correct_gambits, num_correct_gambits = my_gambits, num_my_gambits
  else  
...
-- Toggles engaged_only on/off
function toggle_engaged_only()
  if (engaged_only) then
    engaged_only = false
    notice("ENGAGED_ONLY: OFF")
  else
    engaged_only = true
    notice("ENGAGED_ONLY: ON")
  end
end

-- Toggles claim checks on/off
function toggle_claim_checks()
  if (claim_checks) then
    claim_checks = false
    notice("CLAIM CHECKS: OFF")
  else
    claim_checks = true
    notice("CLAIM CHECKS: ON")
  end
end
...
-- Checks an incoming cta_ticket and renables actions
-- if we haven't chosen a new action since the creation
-- of the ticket.
function check_cta_ticket(incoming_cta_ticket)
  if (can_take_action == false) then
    if (incoming_cta_ticket == cta_ticket) then
      cta(true)
    end
  end
end
...
function process_selector(selector, trigger, trigger_arg, gambit_num)
  local valid_self_triggers = S{"STATUS","NOT_STATUS","NOT_ENGAGED","NOT_ASSISTING","TP >=","HPP <=","HPP >","MPP <=","MA_READY","JA_READY",
    "MANEUVERS <","FIRE_MANEUVERS <","ICE_MANEUVERS <","WIND_MANEUVERS <","EARTH_MANEUVERS <","THUNDER_MANEUVERS <","WATER_MANEUVERS <","LIGHT_MANEUVERS <","DARK_MANEUVERS <",
    "SHADOWS <","MOVES <","MOVES >=","NO_PET","PET_HPP <=","PET_IDLE","CAN_SC","CAN_MB"}
  ...
  local triggers_with_raw_args = S{"HPP <=","HPP >","MPP <=","TP >=","NOT_ENGAGED","NOT_ASSISTING","NOT_TAGGED",
      "MANEUVERS <","FIRE_MANEUVERS <","ICE_MANEUVERS <","WIND_MANEUVERS <","EARTH_MANEUVERS <","THUNDER_MANEUVERS <","WATER_MANEUVERS <","LIGHT_MANEUVERS <","DARK_MANEUVERS <",
      "SHADOWS <","MOVES <","MOVES >=","NO_PET","PET_HPP <=","PET_IDLE","CAN_SC","CAN_MB"}
...
windower.register_event("zone change", function(new_id, old_id)
  ...
  coroutine.close(movement_update_loop)
...
skillchain_names = {"Light","Darkness","Gravitation","Fragmentation","Distortion","Fusion","Compression","Liquefaction","Induration","Reverberation","Transfixion","Scission","Detonation","Impaction","Radiance","Umbra"}
...
skillchain_MBs = {
  ...
  ["Radiance"] = S{"Fire","Light","Lightning","Wind"},
  ["Umbra"] = S{"Darkness","Earth","Ice","Water"},
}
...
sphere_trusts = S{'Moogle','Sakura','Star Sibyl','Kuyin Hathdenna','Kupofried','Cornelia','Brygid'}

status_locked = false
can_take_action, cta_ticket = true, 0
gambits_enabled = true
engaged_only = false
callback = false
claim_checks = true
...
if files.exists('my_gambits/'.. my_name ..'.lua') then
  my_gambits, num_my_gambits = load_gambits('my_gambits/'.. windower.ffxi.get_player().name)
else
  my_gambits, num_my_gambits = load_gambits({})
end
