...
V232  - 4/24/2016 :
        + Mirror mode toggle added for enabling commands.lua.
...
V230  - 4/20/2016 :
        ...
        + Added functionality to support Commands.lua library.
            ...
            > Follows strict sequence: 1 / 2 / 3 / 4
...
function get_sets()
    ...
    send_command('bind ^!@f3 gs c _modeMIRROR')      -- Toggle on, and off mirror account mode. This makes all alt. accounts mirror all commands added in commands.lua library.
    send_command('bind @f gs c send start follow')   -- Send follow command to all accounts in party.
    send_command('bind @s gs c send stop follow')    -- Send stop command to all accounts in party.
    ...
    _MIRROR      = false;   -- Enables mirror function support across all accounts.
...
function self_command(command)    
    ...
    elseif command == '_modeMIRROR' then
        
        -- Keeps track of variable.
        if _MIRROR == false then
            _MIRROR = true
            add_to_chat(200, '<< Mirror Mode Enabled. >>')
            
        elseif _MIRROR == true then
            _MIRROR = false
            add_to_chat(200, '<< Mirror Mode Disabled. >>')
